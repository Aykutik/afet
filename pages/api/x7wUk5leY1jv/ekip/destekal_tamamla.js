...
export default async (req, res) => {
  if (req.method === "POST") {
    ...
    if (durum === "tamamla") {
      ...
      const result =
        await prisma.$queryRaw`UPDATE destekal set durum="tamamlandı",kapatan=${kapatan},kapatanaciklama=${kapatanaciklama},kapandı=1,kapatmatarihi=${kapatmatarihi} where id=${getir}`;
